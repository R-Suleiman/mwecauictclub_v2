...
class AdminController extends Controller
{
    ...
    {
        $department = Department::with('admins.user')->findOrFail($id);
        return view('admin.department_show', compact('department'));
    }
    ...
    public function updateDepartment(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'dept_cover' => ['nullable', 'image', 'mimes:jpeg,png,jpg,gif', 'max:9048'],
        ], [
            ...
            'dept_cover.image' => 'Cover must be an image file',
        ]);
    ...
    public function departmentDestroy($id)
{
    ...
    public function newDepartment(Request $request)
    {
        ...
        if ($request->hasFile('dept_cover')) {
            $path = $request->file('dept_cover')->store('public/departments');
            $newDepartment->dept_cover = str_replace('public/', 'storage/', $path);
        }
